...
const Library = () => {
    ...
    useEffect(()=>{
        fetch(`${import.meta.env.VITE_REACT_APP_API_URL}/api/v1/student/getLibraryRecords`,{
            headers: {
                ...
                <CardWrapper4>
                    <table className="min-w-[350px] w-full border-spacing-5">
                        <thead className={h3Style+" border-b-4 h-16"}>
                            <tr>
                                <th>Id</th>
                                <th>{screenWidth>1020? "Book No": "B. No"}</th>
                                <th>Taken on</th>
                                <th>Due on</th>
                            </tr>
                        </thead>
                                    ...
                                    libraryRecords.map((row, index)=>{
                                        return (
                                            <tr key={index} className="h-16 text-center border-b-2">
                                                <td>{row.id}</td>
                                                <td>{row.bookNo}</td>
                                                <td>{new Date(row.takenOn).toLocaleDateString("en-GB", {
                                                    day: '2-digit',
                                                    month: 'long',
                                                    year: 'numeric'
                                                })}</td>
                                                <td>{new Date(row.dueDate).toLocaleDateString("en-GB", {
                                                    day: '2-digit',
                                                    month: 'long',
                                                    year: 'numeric'
                                                })}</td>
